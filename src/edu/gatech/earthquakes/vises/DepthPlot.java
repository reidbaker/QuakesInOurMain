...
public class DepthPlot extends Multi implements Filterable, Interactable {
    ...
    private void drawAxes(PApplet parent) {
        ...
        for (int i = 0; i < depthRange[1]; i += 5) {
            parent.line(x + buffer - 2, y + buffer + i * depthPixelOffset, x
                    + w - buffer, y + buffer + i * depthPixelOffset);
            parent.pushMatrix();
            parent.translate(x + 2 * buffer / 3, y + buffer + i
                    * depthPixelOffset);
        ...
        if (mX > x && mX < x + w && mY > y && mY < y + h) {
            ...
            for (int i = 0; i < depths.length; i++) {
                ...
                if (!found) {
                    highlightedIndex = -1;
                    Controller
                            .applyBrushing(new DataSet(new HashSet<DataRow>()));
                }
